    ...
    if (!is_home() && !is_admin()) {
        ...
        if (is_category()) {
            ...
            if ($post->post_parent != 0) {
                ...
                foreach ($ancestors as $ancestor) {
                    $str2.='<div itemscope itemtype="http://data-vocabulary.org/Breadcrumb"><a href="' . get_permalink($ancestor) . '" itemprop="url"><span itemprop="title">' . get_the_title($ancestor) . '</span></a></div>';
                }
            } else {
            ...
            $str.='<div itemscope itemtype="http://data-vocabulary.org/Breadcrumb"><a href="' . get_category_link($cat->term_id) . '" itemprop="url"><span itemprop="title">' . $cat->cat_name . '</span></a> &gt; ' . get_the_title() . '</div>';
        } else {
